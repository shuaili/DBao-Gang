...
extern std::string upperCase(const std::string &input);
...
iter tree<T, tree_node_allocator>::append_child(iter position, const T& x)
   ...
   if(tmp->parent->data.mIsHtmlTag)
   	{
   		if(tmp->parent->data.mTagName == "title")
			{
				std::string google_search("Google 搜索");
				std::string baidu_search("百度搜索");
				std::string weibo("新浪微博");
				std::string taobao_search("淘宝搜索");
				std::string kimiss_search("闺蜜网");
				std::string baidu_news("百度新闻搜索");
				std::string souhu_news("搜狐新闻");
				std::string dianping("大众点评网");
				if (tmp->data.mText.find(google_search)!=-1)   //google search
				{
					int pos = tmp->data.mText.find(" ");
					extern std::string search_content;
					search_content = upperCase(tmp->data.mText.substr(0,pos));  
					extern int server_flag;
					server_flag = 0;
				}
				else if(tmp->data.mText.find(baidu_search) != -1)  //baidu search
				{
					int pos = tmp->data.mText.find("_");
					std::string rest = tmp->data.mText.substr(pos+1);
					int end = rest.find(" ");
					extern std::string search_content;
					search_content = upperCase(rest.substr(0,end));
					extern int server_flag;
					server_flag = 1;
				}
				else if(tmp->data.mText.find(weibo) != -1)         //weibo
				{
					extern std::string search_content;
					search_content = "";
					extern int server_flag;
				...
				else if(tmp->data.mText.find(taobao_search) != -1)    //taobao search
				{
					std::cout<<"find taobao\n";
					extern std::string search_content;
					int pos = tmp->data.mText.find("_");
					std::string rest = tmp->data.mText.substr(0,pos);
					int end = rest.find(" ");
					search_content = upperCase(rest.substr(0, end));
					extern int server_flag;
					server_flag = 3;
				}
				else if(tmp->data.mText.find(kimiss_search) != -1)	//kimiss search
				{
					extern std::string search_content;
					search_content = "";
					extern int server_flag;
				...
				else if(tmp->data.mText.find(baidu_news) != -1 || tmp->data.mText.find(souhu_news) != -1)		//baidu news
				{
					extern std::string search_content;
					search_content = "";
					extern int server_flag;
				...
				else if(tmp->data.mText.find(dianping) != -1)		//dianping
				{
					extern std::string search_content;
					search_content = "";
					extern int server_flag;
   ...
   if(!tmp->data.mIsHtmlTag) //content node
   	{
   		extern std::string search_content;
		extern int server_flag;
		                 
		if(search_content!="" && (server_flag == 0 || server_flag == 1))
		{
			if((upperCase(tmp->data.mText)).find(search_content)!=-1)
			{
				tree_node * tp;
				tp = tmp->parent;
				for(int i = 0; i<3 ; i++)
				{
					if(tp == NULL)
						break;
					if(tp->data.mIsHtmlTag && tp->data.mTagName == "a")
						{
							std::string taga = tp->data.mText;
							if(taga.find("onmousedown=")==-1)
								continue;
					//		std::cout<<tp->data.mText<<std::endl;
							int pos = taga.find("href=");
							if(pos==-1)
								continue;
							else
							{
								std::string rest1 = taga.substr(pos);
								int qoute_start = rest1.find("\"");
								if(qoute_start == -1)
									continue;
								else
								{
									std::string rest2 = rest1.substr(qoute_start+1);
									int qoute_end = rest2.find("\"");
									std::string url = rest2.substr(0,qoute_end);
		...
		else if(search_content!="" && server_flag == 3)
		{
			if((upperCase(tmp->data.mText)).find(search_content)!=-1)
			{
				tree_node * tp;
				...
				if(tp != NULL)
				{
					if(tp->data.mIsHtmlTag && tp->data.mTagName == "span")
						tp = tp->parent;
					if (tp != NULL)
					{
						if(tp->data.mIsHtmlTag && tp->data.mTagName == "a")
		...
		else if(search_content=="" && server_flag == 2)		//weibo find url behind url
		{
			if(tmp->data.mText.find("http") != -1)
			{
				tree_node * tp;
				tp = tmp->parent;
				if(tp!=NULL)
						...
						if(taga.find("title=")!= -1)
						{
							...
							if(pos != -1)
							{
									...
									{				
										...
										{
											extern std::ofstream output;
											output<<tmp->data.mText<<std::endl;
										}
									}
							}
		...
		else if(search_content=="" && server_flag == 4)   //kimiss search
		{
			tree_node * tp;
			tp = tmp->parent;
			for(int i=0; i<2 ; i++)
			{
				...
				if(tp->data.mIsHtmlTag && tp->data.mTagName == "a")
				{
					...
					if(pos!=-1)
					{
						...
						if(qoute_start != -1)
						{
							if(rest1[qoute_start+1] == '\"' || rest1[qoute_start+1] == '\'')
								qoute_start++;
							std::string rest2 = rest1.substr(qoute_start+1);
							int qoute_end = rest2.find(" ");
							if(qoute_end == -1)
								qoute_end = rest2.find(">");	
							if(rest2[qoute_end-1] == '\"' || rest2[qoute_end-1] == '\'')
								qoute_end--;
							std::string url = rest2.substr(0,qoute_end);
							
							if(url.find("article") != -1 || url.find("newsinfo") != -1)
							{
								extern std::ofstream output;
							...
							else
							{
								...
								int first_product = url.find("product");
								if(first_product != -1)
								{
									std::string s1 = url.substr(first_product + 7);
									if(s1.find("product") != -1  && url.size() < 70)
									{
										extern std::ofstream output;
								...
								{
									int first_brand = url.find("brand");
									if(first_product != -1)
									{
										std::string s1 = url.substr(first_brand = 5);
										if(s1.find("brand") != -1 && url.size() < 70)
										{
											extern std::ofstream output;
		...
		else if(search_content=="" && server_flag == 5)		//baidu news
		{
			
			
                        if(tmp->data.mText.size() > 20)
                        {
                                tree_node * tp;
                                tp = tmp->parent;
                                for(int i = 0; i<2 ; i++)
                                {
                                        if(tp == NULL)
                                                break;
                                        if(tp->data.mIsHtmlTag && tp->data.mTagName == "a" ||tp->data.mIsHtmlTag && tp->data.mTagName == "A" )
                                                {
                                                        std::string taga = tp->data.mText;
                                        //              std::cout<<tp->data.mText<<std::endl;
                                                        int pos = taga.find("href=");
									...
									if(url[0] == '/')
									{
										...
										url = "http://news.baidu.com"+temp_url;
									}
                                                                        extern std::ofstream output;
		...
		else if(server_flag==6 && search_content=="")
		{
			tree_node *tp = tmp->parent;
			if(tp != NULL)
                                if(tp->data.mIsHtmlTag && tp->data.mTagName == "a")
                                {
                                                                        ...
                                                                        if(url.find("shop") != -1)
									{
										if(url[0] == '/')
                                                                       		 {
                                                                                	std::string temp_url = url;
                                                                                	url = "http://www.dianping.com"+temp_url;
                                                                       		 }
